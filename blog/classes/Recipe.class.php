...
class Recipe
{
    ...
    /**
     * @var string The JSON of the ingredients
     */
    public $ingredients = "";
    ...
    public function __construct($data=array()){
        ...
        if (isset($data['ingredients'])) $this->ingredients = $data['ingredients'];
    ...
    public function storeFormValues ($params) {
        ...
        if(isset($params['ingredients'])){
            $this->ingredients = json_encode($params['ingredients'], JSON_FORCE_OBJECT);
        }
    }
    ...
    public static function getById( $id ) {
        ...
        $sql = "SELECT *, UNIX_TIMESTAMP(publicationDate) AS publicationDate FROM recipes WHERE id = :id";
        $st = $conn->prepare( $sql );
    ...
    public static function getList( $numRows=1000000 ) {
        $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
        $sql = "SELECT SQL_CALC_FOUND_ROWS *, UNIX_TIMESTAMP(publicationDate) AS publicationDate FROM recipes
    ...
    public function insert() {
        ...
        if ( !is_null( $this->id ) ) trigger_error ( "Recipe::insert(): Attempt to insert a post object that already has its ID property set (to $this->id).", E_USER_ERROR );

        // Insert the post
        $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
        $sql = "INSERT INTO recipes ( publicationDate, title, summary, content, ingredients, imageExtension ) VALUES ( FROM_UNIXTIME(:publicationDate), :title, :summary, :content, :ingredients, :imageExtension )";
        $st = $conn->prepare ( $sql );
        $st->bindValue( ":publicationDate", $this->publicationDate, PDO::PARAM_INT );
        $st->bindValue( ":title", $this->title, PDO::PARAM_STR );
        $st->bindValue( ":summary", $this->summary, PDO::PARAM_STR );
        $st->bindValue( ":content", $this->content, PDO::PARAM_STR );
        $st->bindValue(":ingredients", $this->ingredients, PDO::PARAM_STR);
    ...
    public function update() {
        ...
        if ( is_null( $this->id ) ) trigger_error ( "Recipe::update(): Attempt to update a post object that does not have its ID property set.", E_USER_ERROR );

        // Update the Post
        $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
        $sql = "UPDATE recipes SET publicationDate=FROM_UNIXTIME(:publicationDate), title=:title, summary=:summary, content=:content, ingredients=:ingredients, imageExtension=:imageExtension WHERE id = :id";
        $st = $conn->prepare ( $sql );
        $st->bindValue( ":publicationDate", $this->publicationDate, PDO::PARAM_INT );
        $st->bindValue( ":title", $this->title, PDO::PARAM_STR );
        $st->bindValue( ":summary", $this->summary, PDO::PARAM_STR );
    ...
    public function delete() {
        ...
        // Does the post object have an ID?
        if ( is_null( $this->id ) ) trigger_error ( "Recipe::delete(): Attempt to delete a post object that does not have its ID property set.", E_USER_ERROR );

        // Delete the post
        $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
        $st = $conn->prepare ( "DELETE FROM recipes WHERE id = :id LIMIT 1" );
        $st->bindValue( ":id", $this->id, PDO::PARAM_INT );
        $st->execute();
        $conn = null;
    }
    ...
    /**
     * Stores and sizes images for fullsize and thumbnail in separate folders
     *
     * @param $image
     */
    ...
    public function deleteImages() {
        ...
        foreach (glob( RECIPE_IMAGE_PATH . "/" . IMG_TYPE_FULLSIZE . "/" . $this->id . ".*") as $filename) {
            if ( !unlink( $filename ) ) trigger_error( "Recipe::deleteImages(): Couldn't delete image file.", E_USER_ERROR );
        }

        // Delete all thumbnail images for this article
        foreach (glob( RECIPE_IMAGE_PATH . "/" . IMG_TYPE_THUMB . "/" . $this->id . ".*") as $filename) {
            if ( !unlink( $filename ) ) trigger_error( "Recipe::deleteImages(): Couldn't delete thumbnail file.", E_USER_ERROR );
        }
    ...
    public function getImagePath( $type=IMG_TYPE_FULLSIZE ) {
        return ( $this->id && $this->imageExtension ) ? ( RECIPE_IMAGE_PATH . "/$type/" . $this->id . $this->imageExtension ) : false;
    }
